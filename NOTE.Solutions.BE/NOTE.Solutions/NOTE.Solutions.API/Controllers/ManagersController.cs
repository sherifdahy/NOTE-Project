...
public class ManagersController(IManagerService managerService) : ControllerBase
{
    ...
    {
        var result = await _managerService.ToggleStatusAsync(managerId,cancellationToken);
        return result.IsSuccess ? NoContent() : result.ToProblem();
    }

    [HttpPut("{managerId:int}")]
    public async Task<IActionResult> Update(int managerId,ManagerRequest request,CancellationToken cancellationToken = default)
    {
        var result = await _managerService.UpdateAsync(managerId,request,cancellationToken);
        return result.IsSuccess ? NoContent() : result.ToProblem();
    }
}
