...
public class ActiveCodesController(IActiveCodesService activeCodeService) : ControllerBase
{
    ...
    public async Task<IActionResult> GetAll([FromQuery] RequestFilters filters, [FromQuery] bool includeDisabled,CancellationToken cancellationToken)
    {
        var result = await activeCodeService.GetAllAsync(filters,includeDisabled,cancellationToken);
    ...
    {
        var result = await activeCodeService.ToggleStatusAsync(id);
        return result.IsSuccess ? NoContent() : result.ToProblem();
    }
}
