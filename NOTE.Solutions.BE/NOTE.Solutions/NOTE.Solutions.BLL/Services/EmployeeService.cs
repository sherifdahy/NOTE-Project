...
public class EmployeeService(IUnitOfWork unitOfWork,UserManager<ApplicationUser> userManager) : IEmployeeService
{
    ...
    {
        var employee = await _unitOfWork.Employees.FindAsync(x=>x.Id == id,null,cancellationToken);
    ...
    {
        var result = await _unitOfWork.Employees.FindAllAsync(x=> x.BranchEmplyees.Any(x=>x.BranchId == branchId), null, cancellationToken);
