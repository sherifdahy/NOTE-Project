...
import { RegisterRequest } from '../models/authentication/requests/register-request';
...
export class AuthService {
  ...
  register(request : RegisterRequest) : Observable<AuthResponse>{
    return this.apiCall.post<AuthResponse>('api/auth/register-company',request).pipe(
      retry(3),
      catchError((response)=>{
